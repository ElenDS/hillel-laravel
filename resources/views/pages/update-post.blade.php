...
@section('main')
    <div class="container d-block">
        <p>{{$textLength}} characters, {{$wordNumber}} words</p>
        <form action="/admin/posts/{{$post->id}}" method="post" class="m-5">
            ...
            <div class="form-group pb-3">
                ...
                <textarea id="text" class="form-control @error('text') is-invalid @enderror" name="text">{{$post->text}}</textarea>
            </div>
            ...
            @foreach($categories as $category)
                ...
                <div class="form-check">
                    <input class="form-check-input" type="radio" value="{{$category->id}}" id="{{$category->name}}"
                           name="category_id" @if($category->id === $post->category_id) checked @endif>
            ...
            @foreach($tags as $tag)
                ...
                <div class="form-check">
                    <input class="form-check-input" type="checkbox" value="{{$tag->id}}" id="{{$tag->name}}"
                           name="tags[]" @if($post->tags->contains('id', $tag->id)) checked @endif>
